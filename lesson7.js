...
//***********************ДОБАВИЛ
var bomb_timer; //Таймер препятствия
...
function startGame() {
  ...
  //***********************ДОБАВИЛ
  bomb_timer = setInterval(createBomb, 5000); //каждые 5 сек запускаем препятствие
}
...
function move() {
  ...
  if (!isSnakeUnit(new_unit) && !isBombFail(new_unit)) {
  ...
  //Проверяем дошла ли змека до границы
}

//перенос змейки
function teleport(coord_y, coord_x) {
  var unit;
  if (direction == "x-") {
...
function haveFood(unit) {
  ...
  if (unit_classes.includes("food-unit")) {
    ...
    createFood();
    score++;
    document.getElementsByClassName("point")[0].innerHTML = score;
  }
  return check;
}
...
function createBomb() {
  ...
  while (!bombCreated) {
    //пока бомбу не создали не создали
    // рандом
